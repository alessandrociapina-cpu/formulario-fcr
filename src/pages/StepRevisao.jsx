        ...
        <div className="mx-4 mt-4 bg-amber-50 border border-amber-200 rounded-xl p-3 flex gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-amber-700 mt-0.5">
              Os itens {missing.join(', ')} estão com "NÃO ATENDE" mas sem justificativa preenchida.
            </p>
          </div>
        </div>
      )}
              ...
              <div key={g.id} className="flex items-center justify-between px-4 py-3 border-b border-slate-100 last:border-0">
                ...
                {skipped ? (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-bold w-12 text-right ${scoreBg(s.ratio).replace('bg-', 'text-')}`}>
                      {scoreLabel(s.ratio)}
                    </span>
                  </div>
                ) : (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          ...
          <div className="p-4 grid grid-cols-2 gap-2 text-xs">
            {[
              ...
              ['Município', state.cabecalho.municipio],
            ].map(([k, v]) => (
              <div key={k}>
